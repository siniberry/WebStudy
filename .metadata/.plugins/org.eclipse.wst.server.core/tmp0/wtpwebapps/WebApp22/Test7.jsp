...
<div>
	...
	<p>(page | request | session | application)
		범위의 변수(속성)를 설정한다.</p>
	<p>『c:remove』 은 JSP의 removeAttribute()와 같은 역할을 수행한다.</p>
	<p>(page | request | session | application)
		범위의 변수(속성)를 제거한다.</p>
	<p> scope 속성이 생략될 경우 기본 값(default)는 page 이다.</p>
</div>
...
<div>
	...
	<c:if test="${!empty param.su1 }">
		...
		<%--    변수를 새로 선언하여 초기화 하는 기능을 수헹하게 되며--%>
		<%--    해당 스코프에 이미 지정된 변수가 존재할 경우--%>
		<%--     그 변수의 값을 다시 초기화하는 효과가 적용된다.--%>
		
		<%-- String result = "1" 과 같다 --%>
		<%-- <c:set var="result" value="2"></c:set>라고 했을 때 오류나는 것이 아닌 String result = "2"로 처리된다.--%>
